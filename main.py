...
def main():
  ...
  # Get some information about this computer...
  txtcmd = None
  if "--skiptext" not in sys.argv:
    try:
      subprocess.check_call(["pdf2txt"])
    except OSError: # not found
      try:
        subprocess.check_call(["pdf2txt.py"])
      except OSError: # not found
        logging.critical("Can't find a pdf2txt script. Use --skiptext to work around.")
  ...
  try:
    for fname, categories, tmp_path in pending_upload:
      ...
      for png_fname in sorted(os.listdir(tmp_path), key = sortpng):
        ...
        if txtcmd != None:
          extract = "%s -p %d -t text \"%s\"" % (txtcmd, counter, fname)
